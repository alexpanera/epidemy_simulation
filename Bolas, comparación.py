# -*- coding: utf-8 -*-
"""
Created on Fri Apr  3 11:30:47 2020
...
Npasos=12000 #'tiempo' que dura la simulación
pasosrep = np.arange(0,Npasos,1) #se utiliza después para representar 
pasos=100 #número de pasos entre representación y representación
...
duracion_enfermedad = 2000 #número de pasos que dura la enfermedad
duracion_inmunidad = 30000  #número de pasos que dura la inmunidad
curados_muerte = 90000 #número de habitantes que se curan por cada habitante muerto
porcentaje_confinados = 0 #porcentaje de la población que está confinado
rendijax = 2.5 #posición X de la rendija
apertura = 0.06 #longitud de la apertura de la rendija
porcentaje_cuarentena = 0.2 #porcentaje de la población en cuarentena
...
nrep = 1 #número de veces que lo queremos ejecutar. Es útil si queremos calcular la media de varias ejecuciones
...
infect_media_1 = np.mean(infect_todos,axis=0)
...
#representamos la media de los infectados de cada simulación
infect_media_2 = np.mean(infect_todos,axis=0)
curados_media = np.mean(curados_todos,axis=0)
sanos_media = np.mean(sanos_todos,axis=0)
muertos_media = np.mean(muertos_todos,axis=0)

plt.figure()
plt.plot(pasosrep,infect_media_2,label='infectados',color='r')
plt.plot(pasosrep,curados_media,label='curados',color='b')
plt.plot(pasosrep,sanos_media,label='sanos',color='g')
plt.title('Confinamiento')
plt.xlabel('nº pasos')
...
#representamos la media de los infectados de cada simulación
infect_media_3 = np.mean(infect_todos,axis=0)
curados_media = np.mean(curados_todos,axis=0)
sanos_media = np.mean(sanos_todos,axis=0)
muertos_media = np.mean(muertos_todos,axis=0)

plt.figure()
plt.plot(pasosrep,infect_media_3,label='infectados',color='r')
plt.plot(pasosrep,curados_media,label='curados',color='b')
plt.plot(pasosrep,sanos_media,label='sanos',color='g')
...
plt.figure()
plt.title('Comparación infectados')
plt.plot(pasosrep,infect_media_1,label='Cuarentena 20%, apertura 0.06')
plt.plot(pasosrep,infect_media_2,label='Movimiento libre')
plt.plot(pasosrep,infect_media_3,label='Confinados 50%')
plt.xlabel('nº pasos')
...
a = inter.trapz(infect_media_1, pasosrep)
b = inter.trapz(infect_media_2, pasosrep)
c = inter.trapz(infect_media_3, pasosrep)

integ_infect_media_1 = infect_media_1 - 50
integ_infect_media_2 = infect_media_2 - 50
integ_infect_media_3 = infect_media_3 - 50
...
for i in range(Npasos):
    if integ_infect_media_1[i] > 0:
        integ_1 = np.append(integ_1,integ_infect_media_1[i])
    if integ_infect_media_2[i] > 0:
        integ_2 = np.append(integ_2,integ_infect_media_2[i])
    if integ_infect_media_3[i] > 0:
        integ_3 = np.append(integ_3,integ_infect_media_3[i])
...
a1 = inter.trapz(integ_1)
a2 = inter.trapz(integ_2)
a3 = inter.trapz(integ_3)
